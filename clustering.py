...
#print "\nmodified word_frs (after tf x idf):"
for i in range(no_of_papers):
    #print titles[i], 'by', authors[i]
    for e in word_frs[i]:
        do_nothing = True
    ...
    if (i == find(i)):
        ...
        for j in range(i, no_of_papers):
            if (find(j) == i):
                ...
                title_in_cluster += titles[j]
                title_in_cluster += ' * '
                
        count += 1
        print ''
    if (len (title_in_cluster) != 0):
        f.write(title_in_cluster+'\n')
f.close() 
